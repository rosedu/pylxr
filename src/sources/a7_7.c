...
int main(int argc, char **argv) {
	...
	strcpy(filename_o, "");
	int nrc = strchr(filename, '.') - filename;
	strncpy(filename_o, filename, nrc);
	filename_o[nrc] = 0;
	strcpy(filename_o+strlen(filename_o), "_binary");
	strcpy(filename_o+strlen(filename_o), strchr(filename, '.'));
